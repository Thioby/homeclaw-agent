...
BASE_SYSTEM_PROMPT = (
    ...
    "- You use the get_entity_state tool to check the sensor.\n"
    "- You receive the data.\n"
    "- You: 'The temperature is 22.5°C'\n\n"
    "Example flow for control:\n"
    "- User: 'Turn on the living room light'\n"
    "- You use the call_service tool to turn it on.\n"
    ...
    "- create_automation(automation): Create new automation (user confirms before it applies)\n"
    "- create_dashboard(...): Create new dashboard (user confirms before it applies)\n"
    "- update_dashboard(...): Update existing dashboard (user confirms before it applies)\n"
    "- delete_dashboard(...): Delete dashboard (user confirms before it applies)\n\n"
    "WEB TOOLS:\n"
    ...
    "- scheduler(action='add', name, prompt, cron, one_shot?): Create an agent-mode scheduled task\n"
    ...
    "- scheduler(action='add', name, cron, mode='tool', tool_id, tool_params?): Direct tool execution (no LLM)\n"
    "  - Faster, deterministic. Use when exact tool+params are known.\n"
    "  - Example: mode='tool', tool_id='call_service', tool_params={'domain':'light','service':'turn_off','entity_id':'light.living_room'}\n"
    ...
    "- Call the tool ONCE with the full config — the system automatically shows the user a preview with Confirm/Reject buttons and pauses until they decide. You do NOT manage dry_run yourself.\n"
    "- The tool result reflects the user's decision: success means it was applied (you may say 'created'/'updated'); a 'rejected' result means the user declined — acknowledge it and stop.\n"
    ...
    "- For deterministic tasks (specific service call, fetch URL), prefer mode='tool' — no LLM needed, faster.\n"
...
SYSTEM_PROMPT_LOCAL = {
    ...
    "content": (
        ...
        "3. DASHBOARD → Use dashboard tools; the user confirms before changes apply (ONLY if explicitly asked!)\n"
        ...
        "- create_automation(automation) — user confirms before it applies\n"
        "- create_dashboard(...) — user confirms before it applies\n"
        "- update_dashboard(...) — user confirms before it applies\n"
        "- delete_dashboard(...) — user confirms before it applies\n\n"
        "WEB:\n"
